...
class BiEncoderTrainer(object):
    ...
    def run_train(self):
        ...
        max_iterations = train_iterator.get_max_iterations()  # batch 数量
        ...
        for epoch in range(self.start_epoch, int(cfg.train.num_train_epochs)):
            ...
            print("cfg.train.num_train_epochs!!!", cfg.train.num_train_epochs)
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            if isinstance(samples_batch, Tuple):
                samples_batch, dataset = samples_batch  # 数据集
            ...
            rolling_train_loss += float(loss.item())
            ...
            if i % log_result_step == 0:
                ...
                # 学习率太小会导致这里打印出来是0，因为float类型保留小数点后面的位数有限
...
def _calc_loss(
    ...
    if distributed_world_size > 1:
        ...
        for i, item in enumerate(global_question_ctx_vectors):
            q_vector, positive_idx, hard_negatives_idxs = item
